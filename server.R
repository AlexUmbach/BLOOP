...
server <- function(input,output,session) {
  ...
  observe({
    ...
    updateSelectInput(session,"read_sortby_axis",choices = c(meta_colnames,"TaxaName"))
    ...
    ## This line queries the metadata and data_long dataframe for the column headers and forwards them to the dropdown menu in the ui, whilst also removing unneeded headers.
    updateSelectInput(session,"b1_color_param",choices = meta_colnames)
    #updateSelectInput(session,"b1_sort_param",choices = colnames(meta_datafile()[,!names(meta_datafile()) %in% c("SampleName")]))
    #updateSelectInput(session,"b1_color_param",choices = colnames(meta_datafile()))
    ...
    #updateSelectInput(session,"b1_sort_axis",choices = colnames(meta_datafile()))
    
    ## Update selection - PCoA plot
  ...
  #### New upload tab ####
{
    ...
    output$main_table = renderDataTable({
      req(input$main_file)
      read.table(file = input$main_file$datapath, fill = TRUE, header = TRUE,sep = "\t")
    })
    ...
    meta_datafile_og = reactive({
      req(input$meta_file)
      read.table(file = input$meta_file$datapath,fill = TRUE, header = TRUE,sep = "\t")
    })
    ...
    meta_datafile = reactive({
      ...
      meta_data_table <- replace(meta_data_table, is.na(meta_data_table), "NA")
      meta_data_table
    })
  ...
  data_long_react = reactive({
    ...
    if(input$is_main_collapsed == "Yes"){
      ...
      data_tran$Consensus.Lineage <- gsub(" ","",data_tran$Consensus.Lineage)
    } else {
      labels <- data_tran$Consensus.Lineage
      labels <- paste(labels,data_tran$rowID,sep="_")
      data_tran$Consensus.Lineage <- gsub(" ","",data_tran$Consensus.Lineage)
    ...
    if (input$remove_prefix == "Yes"){
      ...
      labels<-gsub("(;__)","",labels)
      labels<-gsub("(;$)","",labels)
      labels<-gsub("(;$)","",labels) #This is not a duplicate -- leave it here
    }
    
    ## S## Retrieves the last taxonomy entry (i.e., genus, species):
    full_lineage <- as.data.frame(paste(data_tran$Consensus.Lineage,data_tran$rowID,sep = "_"))
    ...
    cols_to_filter <- c("OTU.ID","Consensus.Lineage", "ReprSequence","rowID","Feature.ID")
    filtered_table <- data_tran[ , !(names(data_tran) %in% cols_to_filter) ]
    ...
    data_long <- reshape2::melt(data_prop, id.vars = c("TaxaName","Taxonomy"), variable.name = as.character("SampleName"),value.name = "Percentage")
    data_long
  })
  ...
  data_filtered_table = reactive({
    ...
    cols_to_filter <- c("OTU.ID", "rowID", "Consensus.Lineage", "ReprSequence","rowID","Feature.ID")
    filtered_table <- unfiltered_table[ , !(names(unfiltered_table) %in% cols_to_filter) ]
  ...
  observeEvent(input$read_start,{
    ...
    data_read_total <- reshape2::melt(data_read_total, variable.name = as.character("SampleName"), value.name = "Total")
    
    ## Add metadata columns
    data_read_total <- left_join(data_read_total,meta_data_table, by = "SampleName", copy = FALSE)
    ...
    data_read_total$SampleName <- as.character(data_read_total$SampleName)
    data_read_total$SampleName <- factor(data_read_total$SampleName, levels=unique(data_read_total$SampleName))
  ...
  data_read_plot = reactive({
    ...
    if(input$box_select == "Bar"){
      read_plot <- ggplot(data = data_read_total, 
                          aes(x=SampleName, y=Total, width = input$read_width))
      
      read_plot <- read_plot + geom_bar(aes(fill=as.factor(eval(parse(text=paste("data_read_total$",input$read_sortby_axis))))),
                                        colour="black",
    ...
    if(input$box_select == "Box"){
      read_plot = ggplot(data = data_read_total, 
                         ...
                         aes(x=as.factor(input$read_sortby_axis), y=Total, width = 2))
      
      
      
      read_plot = read_plot + geom_boxplot(
        aes(fill=as.factor(eval(parse(text=paste("data_read_total$",input$read_sortby_axis))))),
        position = "dodge2",
        alpha = input$read_alpha,
    ...
    #   ## Adds jitter points:
    #   if (isTRUE(box_datapoint == "Jitter")){
    #     box_plot <- box_plot + geom_jitter()
    #   }
    #   ## Adds stacked points:
    #   if (isTRUE(box_datapoint == "Stacked")){
    #     ## Adds stacked data points to your box plots
    #     box_plot<- box_plot + geom_dotplot(binaxis = "y",stackdir ="center",dotsize = 0.25)
    #   }
    # }
    
    ## This is for creating grids/wraps with metadata
    #read_plot<-read_plot + facet_wrap(vars(data_long_bar$SampleType),ncol=2, scales = "free")
    #read_plot<-read_plot + facet_grid(cols= vars(data_long_bar$SampleType),scales = "free")
    ...
    # read_plot<-read_plot + scale_x_discrete(expand = expansion(input$read_yaxis_space),
    #                                         breaks=NULL)
    ...
    read_plot = read_plot + 
      facet_grid(~eval(parse(text=input$read_sortby_axis)),space = "free",scales="free",switch = "both")
    
    
    ##This is if you want to add an error bar
    ...
    ## many of these are self explanatory but can be explained using the ?theme function
  ...
  output$read_download = downloadHandler(
    ...
    content = function(read_file){
      ggsave(read_file, plot = data_read_plot(), device = "pdf", height = as.numeric(input$read_plot_out_h), width = as.numeric(input$read_plot_out_w), units = "px", scale = 4)
    }
  ) 
  
  
  
  #### Bar Plot - NEEDS WORK - should work just like bubble plot ####
  ...
  data_long_bar_filt_re = reactive({
    ...
    bar_rowIDs <- as.data.frame(gsub(".*_","",bar_data_prop$Taxonomy))
    ...
    bar_labels<-gsub("(;Ambiguous__taxa)",";s__Ambiguous_taxa",bar_labels)
    bar_labels<-gsub("(;Ambiguous_taxa)",";s__Ambiguous_taxa",bar_labels)
    ...
    if (input$remove_prefix == "Yes"){
      ...
      bar_labels<-gsub("(;__)","",bar_labels)
      bar_labels<-gsub("(;$)","",bar_labels)
      bar_labels<-gsub("(;$)","",bar_labels) #This is not a duplicate -- leave it here
    }
    ...
    bar_labels<-gsub(".*;","",bar_labels)
    bar_labels<-paste(gsub(".*;$","",bar_labels),bar_rowIDs$rowIDs,sep="_")
    ...
    # bar_data_prop$Taxonomy = as.character(bar_data_prop$Taxonomy)
    
    
    # bar_data_prop$Taxonomy = as.factor(bar_data_prop$Taxonomy)
    
    ## Transform into long form:
    bar_data_long <- reshape2::melt(bar_data_prop, id.vars = c("TaxaName", "Taxonomy"), variable.name = as.character("SampleName"), value.name = "Percentage")
    ...
    bar_data_long = left_join(bar_data_long,unfiltered_table,by = "Taxonomy", copy = TRUE)
    
    
    ## Modify taxonomy names
    bar_data_long$Taxonomy<-gsub("(D_.__)","",bar_data_long$Taxonomy)
    ...
    data_long_bar <- left_join(data_long_bar,meta_data_table, by = "SampleName", copy = TRUE)
    
    # ## Filter above a given threshold, as well as any samples absent from metadata
    # data_long_bar$Percentage <- round(data_long_bar$Percentage, digits = as.numeric(input$bar_cutoff))
    ...
    data_long_bar <- separate(data_long_bar, Sep, c("Domain","Phylum","Class","Order","Family","Genus","Species"), sep=";",remove=TRUE,convert=FALSE)
    ...
    if (is.na(input$taxon_subset) == TRUE){
      ...
      taxon_hits <- grepl(pattern = input$taxon_subset, ignore.case = TRUE, x = data_long_bar_filt$Taxonomy)
      data_long_bar_taxa_filterd <- data_long_bar_filt[taxon_hits,]
      warning("Taxa filtering selected. CONGRATULATIONS")
    }
    
    
    ## NOT WORKING -- DONT KNOW WHY
    # ## Assigns all filter data to taxa category "ZZOther" and provides the appropriate percent relative abundance:
    # if (is.na(input$taxon_subset) == FALSE){
    #   bar_ex_samples <- anti_join(data_long_bar_filt,data_long_bar_taxa_filtered,by = "SampleName")
    #   bar_ex_samples <- as.data.frame(unique(bar_ex_samples$SampleName))
    #   bar_ex_samples$TaxaName <- "ZZOther"
    #   bar_ex_samples$Percentage <- 100
    #   colnames(bar_ex_samples) <- c("SampleName", "TaxaName", "Percentage")
    # }
    
    # ## Assigns all excluded taxa below the set threshold to category "ZZOther" and provides the appropriate relative abundance:
    bar_incl_samples <- aggregate(data_long_bar_filt$Percentage, by=list(data_long_bar_filt$SampleName), FUN = sum)
    bar_incl_samples <- cbind(bar_incl_samples, 100 - bar_incl_samples$x)
    colnames(bar_incl_samples) <- c("SampleName","FiltSum","Percentage")
    bar_incl_samples$TaxaName <- "ZZOther"
    bar_incl_samples$Taxonomy <- "ZZOther"
    bar_incl_samples <- select(bar_incl_samples,-c("FiltSum"))
    data_long_bar_filt <- bind_rows(data_long_bar_filt,bar_incl_samples)
    ...
    data_long_bar_filt$TaxaName <- gsub("_[0-9]*$","",data_long_bar_filt$TaxaName)
    
    # ## Append the metadata
    data_long_bar_filt <- left_join(data_long_bar_filt,meta_data_table, by = "SampleName", copy = FALSE)
    # data_long_bar_filt <- data_long_bar_filt[order(eval(parse(text=paste("data_long_bar_filt$",input$sort_para))),decreasing = FALSE),]
    
    ## Reorder x-axis to follow metadata category in data frame
    data_long_bar_filt$SampleName <- as.character(data_long_bar_filt$SampleName)
    data_long_bar_filt$SampleName <- factor(data_long_bar_filt$SampleName, levels=unique(data_long_bar_filt$SampleName))
    data_long_bar_filt
  })
  ...
  barplot_reactive = reactive({
    ...
    bar_plot <- bar_plot + geom_bar(aes(fill=TaxaName),
                                    colour="black",
                                    size = 0.5,
                                    alpha= input$bar_alpha,
                                    stat="identity",
                                    ## this position_dodge preserves the size of the bar,
                                    ## so that you don't have different sized bars for identical sample names
    ...
    ## Add faceting for sorting
    bar_plot = bar_plot + 
      facet_grid(~eval(parse(text=input$bar_sortby_xaxis)),
                 space = "free",
                 scales="free",
                 switch = "both")
    ...
    ## adds some spacing between the left and right borders. breaks=NULL will remove x-axis labels.
    # bar_plot<-bar_plot + scale_x_discrete(expand = expansion(c(input$bar_leftright,input$bar_leftright)),
    #                                       breaks=NULL)
    
    #bar_plot<-bar_plot + scale_x_discrete(expand = expansion(c(input$bar_leftright,input$bar_leftright)))
    
    nb.cols <- length(unique(eval(parse(text=paste("data_long_bar_filt$","TaxaName")))))
    ...
    ## cuts out the legend title - DEPRECATED
    #bar_plot<-bar_plot + labs(fill = input$bar_legend_label)
    #bar_plot<-bar_plot + xlab(input$bar_xaxis)
    
    bar_plot = bar_plot + ylab(paste("Proportion of affiliated taxa"," ","at"," ",">",eval(parse(text=input$bar_cutoff)),"%"," ","relative"," ","abundance",sep =""))
  ...
  output$bar_download = downloadHandler(
    ...
    content = function(bar_file){
      ggsave(bar_file, plot = barplot_reactive(), device = "pdf", height = as.numeric((input$taxa_plot_out_h)), width = as.numeric((input$taxa_plot_out_w)), units = "px", scale = 4)
    })
  ...
  data_bubble_reactive_new = reactive({
    ...
    ##Metadata and samples must be filtered from the original ASV table before any abundances are used to filter. How am I going to do this? At a sample level I can remove columns of data by matching column names with the metadata SampleName column. This will solve the critical issue with BLOOP. Filtering specific metadata can also be accomplished here by filtering the meta_data_table as we do further down, then using that filtered file to filter the main proportion table. EASY. 
    
    ## Metadata filtering! We must first filter the sample list according to the metadata IF someone chooses filter by a specific category and keyword:
    
    
    
    # This is not working and I don't know why. If the text field is empty, it 
    
    
    # if(is.na(input$b1_meta_group) == FALSE){
    # # sample_hits = meta_data_table %>% filter(Data3 == "Red")
    # meta_data_table_filt = filter(meta_data_table, input$b1_meta_group == eval(parse(text=paste(input$b1_meta_keyword))))
    # meta_sampnames = meta_data_table_filt$SampleName
    # filtered_table = as.data.frame(filtered_table[,names(filtered_table) %in% meta_sampnames])
    # } else {
    # meta_sampnames = meta_data_table$SampleName
    # filtered_table = as.data.frame(filtered_table[,names(filtered_table) %in% meta_sampnames])}
    ...
    #   warning("No metadata category selected. Script will continue without filtering by groups.")
    # } else {
    #   sample_hits <- grepl(pattern = input$b1_meta_keyword, ignore.case = TRUE, x = (eval(parse(text=paste("meta_data_table$",input$b1_meta_group)))))
    #   
    #   data_long_bubble <- data_long_bubble[sample_hits,]
    #   warning("Metadata filtering selected.")
    # }
    ...
    # To get this working, I need to filter the meta_samplist based on it's associated metadata. 
    ...
    B2_labels<-gsub("(;Ambiguous__taxa)",";s__Ambiguous_taxa",B2_labels)
    B2_labels<-gsub("(;Ambiguous_taxa)",";s__Ambiguous_taxa",B2_labels)
    ...
    if (input$remove_prefix == "Yes"){
      ...
      B2_labels<-gsub("(;__)","",B2_labels)
      B2_labels<-gsub("(;$)","",B2_labels)
      B2_labels<-gsub("(;$)","",B2_labels) #This is not a duplicate -- leave it here
    }
    ...
    B2_labels<-gsub(".*;","",B2_labels)
    B2_labels<-paste(gsub(".*;$","",B2_labels),B2_rowIDs$rowIDs,sep="_")
    ...
    ## Transform into long form:
    B2_data_long <- reshape2::melt(B2_data_prop, id.vars = c("TaxaName", "Taxonomy"), variable.name = as.character("SampleName"), value.name = "Percentage")
    ...
    B2_data_long = left_join(B2_data_long,unfiltered_table,by = "Taxonomy", copy = TRUE)
    ...
    data_long_bubble <- dplyr::filter(data_long_bubble, Percentage > 0)
    data_long_bubble <- left_join(data_long_bubble,meta_data_table, by = "SampleName", copy = TRUE)


    
    ## Round percentages to a given decimal places
    data_long_bubble$Percentage <- round(data_long_bubble$Percentage, digits = as.numeric(input$b1_num_dec))
    ...
    if (is.na(input$b1_tax_keyword) == TRUE){
      ...
      taxon_hits <- grepl(pattern = paste(input$b1_tax_keyword), ignore.case = TRUE, x = data_long_bubble$Taxonomy)
    ...
    data_long_bubble <- separate(data_long_bubble, Sep, c("Domain","Phylum","Class","Order","Family","Genus","Species"), sep=";",remove=TRUE,convert=FALSE)
    ...
    if(isTRUE(input$b1_confirm_sort == "Yes")){
       data_long_bubble <- data_long_bubble[with(data_long_bubble,order(eval(parse(text = input$b1_tax_sort)),TaxaName, decreasing = TRUE)),]
       data_long_bubble$TaxaName <- as.character(data_long_bubble$TaxaName)
       data_long_bubble$TaxaName <- factor(data_long_bubble$TaxaName, levels=unique(data_long_bubble$TaxaName))
       data_long_bubble
     }
    data_long_bubble
  })
  ...
  data_bubble_reactive_old = reactive({
    ...
    data_long_bubble <- dplyr::filter(data_long, Percentage > as.numeric(input$b1_ab_thresh))
    data_long_bubble <- left_join(data_long_bubble,meta_data_table, by = "SampleName", copy = TRUE)
    data_long_bubble$Percentage <- round(data_long_bubble$Percentage, digits = as.numeric(input$b1_num_dec))
    ...
    if (is.na(input$b1_tax_keyword) == TRUE){
      ...
      taxon_hits <- grepl(pattern = input$b1_tax_keyword, ignore.case = TRUE, x = data_long_bubble$Taxonomy)
      data_long_bubble <- data_long_bubble[taxon_hits,]
    ...
    ## Set the taxonomy as factors for later ordering:
  ...
  bubble_plot_re = reactive({
    ...
    # ## OKAY THIS WORKS. It records the selected metadata as a string
    # input_value <- eventReactive(eventExpr = input$b1_sort_axis,
    #                              valueExpr = {
    #                                chosen_value <- input$b1_sort_axis
    #                                return(chosen_value)
    #                              })
    # sort_axis_value = input_value()
    
    
    # ## This section turns all metadata into numbers that can be properly sorted with the "reorder" function. Unfortunately, you lose all defining features. 
    # data_long_bubble = as.data.frame(data_long_bubble)
    # dont_mutate = c("SampleName","TaxaName","SampleName","Phylum","Class","Order","Family","Genus","Species","Domain","Percentage","ReprSequence")
    # 
    # data_long_bubble_mod = data_long_bubble %>% mutate_at(vars(-dont_mutate), factor)
    # data_long_bubble_mod = data_long_bubble %>% mutate_at(vars(-dont_mutate), as.numeric)
    
    
    ## Plot the Data:
    bubble_plot <- ggplot(data_long_bubble,
                          aes(x = reorder(SampleName, eval(parse(text = input$b1_sort_axis))),
                              ...
                              fill = as.factor(eval(parse(text=input$b1_color_param))),
                              color = as.factor(eval(parse(text=input$b1_color_param))), size = Percentage),
    ...
    if(input$b1_second_facet == "No"){
      bubble_plot = bubble_plot + 
        ...
        if(input$b1_confirm_sort == "Yes"){
          if((input$b1_facet_side_x == "Top")&(input$b1_facet_side_y == "Right")){
            facet_nested(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free")
          } else {
            if((input$b1_facet_side_x == "Bottom")&(input$b1_facet_side_y == "Left")){
              facet_nested(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free",switch = "both")
            } else {
              if((input$b1_facet_side_x == "Bottom")&(input$b1_facet_side_y == "Right")){
                facet_nested(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free",switch = "x")
              } else {
                facet_nested(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free",switch = "y")
              }
            }
          }
        } else {
          if(input$b1_facet_side_x == "Top"){
            facet_nested(~eval(parse(text=input$b1_sort_param)),space = "free",scales="free")
          } else {
            facet_nested(~eval(parse(text=input$b1_sort_param)),space = "free",scales="free",switch = "x")
          }}
    }
    ...
    if(input$b1_second_facet == "Yes"){
      bubble_plot = bubble_plot + 
        ...
        if(input$b1_confirm_sort == "Yes"){
          if((input$b1_facet_side_x == "Top")&(input$b1_facet_side_y == "Right")){
            facet_nested(eval(parse(text=input$b1_tax_sort)) ~ eval(parse(text=input$b1_sort_param))+eval(parse(text=input$b1_second_facet_meta)),space = "free",scales="free")
          } else {
            if((input$b1_facet_side_x == "Bottom")&(input$b1_facet_side_y == "Left")){
              facet_nested(eval(parse(text=input$b1_tax_sort)) ~ eval(parse(text=input$b1_sort_param))+eval(parse(text=input$b1_second_facet_meta)),space = "free",scales="free",switch = "both")
            } else {
              if((input$b1_facet_side_x == "Bottom")&(input$b1_facet_side_y == "Right")){
                facet_nested(eval(parse(text=input$b1_tax_sort)) ~ eval(parse(text=input$b1_sort_param))+eval(parse(text=input$b1_second_facet_meta)),space = "free",scales="free",switch = "x")
              } else {
                facet_nested(eval(parse(text=input$b1_tax_sort)) ~ eval(parse(text=input$b1_sort_param))+eval(parse(text=input$b1_second_facet_meta)),space = "free",scales="free",switch = "y")
              }
            }
          }
        } else {
          if(input$b1_facet_side_x == "Top"){
            facet_nested(~eval(parse(text=input$b1_sort_param))+eval(parse(text=input$b1_second_facet_meta)),space = "free",scales="free")
          } else {
            facet_nested(~eval(parse(text=input$b1_sort_param))+eval(parse(text=input$b1_second_facet_meta)),space = "free",scales="free",switch = "x")
          }}
    }
    
    
    #     if((input$b1_facet_side_x == "Top")&(input$b1_facet_side_y == "Left")){
    #       facet_grid(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free",switch = "y")
    #     }} else {
    #       if((input$b1_facet_side_x == "Top")&(input$b1_facet_side_y == "Right")){
    #         facet_grid(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free")
    #       }} else
    #   if(input$b1_facet_side_x == "Top"){
    #     facet_grid(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free")
    #   } else {
    #     facet_grid(eval(parse(text=input$b1_tax_sort))~eval(parse(text=input$b1_sort_param)),space = "free",scales="free",switch = "both")
    #   }
    # }
    #   
    ...
    if(input$b1_incl_percent == "Yes"){
      bubble_plot <- bubble_plot + 
        ...
        geom_text(aes(label = Percentage), colour= "black", size= 3.0) +
        theme(plot.title = element_text(hjust = 0.5)) +
        scale_size_area(max_size = 15) +
        ggtitle("") + xlab("") + 
        scale_size_area(max_size = 15) + 
        ylab(paste("Taxonomic affiliation"," ","at"," ",">",eval(parse(text=input$b1_ab_thresh)),"%"," ","relative"," ","abundance",sep =""))
    } else {
      bubble_plot <- bubble_plot + 
        geom_point(shape = 21, alpha=0.8) +
        ggtitle("") + xlab("") + 
  ...
  output$b1_bubble_download = downloadHandler(
    ...
    content = function(bubble_file){
      ggsave(bubble_file, plot = bubble_plot_re(), device = "pdf", height = as.numeric(input$b1_plot_out_h), width = as.numeric(input$b1_plot_out_w), units = "px", scale = 4)
    }
  )   
  ...
  pcoa_data_main_re = reactive({
    ...
    ## Simple taxonomy corrections
    main_data_table$Consensus.Lineage <- gsub(" ","",main_data_table$Consensus.Lineage)
    ...
    ## ## Retrieves the last taxonomy entry (i.e., genus, species):
    full_lineage <- data.frame(TaxaName = paste(main_data_table$Consensus.Lineage,main_data_table$rowID,sep = "_"))
    last_lineage <- data.frame(TaxaName = paste(gsub(".*;","",labels),main_data_table$rowID, sep="_"))
    ...
    sample_info_data = main_data_table[,colnames(main_data_table) %in% c("Consensus.Lineage","Feature.ID","rowID","ReprSequence","last_taxon")]
    main_data_table
    main_data_table
  })
  
  
  srs_react = reactive({ ## Set an option to select SRS, rarefy, or none (default SRS)
    main_data_table = pcoa_data_main_re()
    meta_data_table = meta_datafile()
    # Set the seed for reproducibility (optional)
    srs_depth = input$pcoa_srs_depth
    srs_table = main_data_table[,!colnames(main_data_table) %in% c("Consensus.Lineage","Feature.ID","rowID","ReprSequence","last_taxon")]
  ...
  srs_proportion_react = reactive({
    ...
    # Remove empty rows
    proportion_table <- proportion_table[rowSums(proportion_table) > 0, ]
  ...
  bc_pcoa_react = reactive({
    ...
    bray_curtis <- as.matrix(vegdist(proportion_table, method = "bray"))
  ...
  pcoa_coords_react = reactive({
    ...
    pcoa_df <- data.frame(PCoA1 = pcoa_coords[, 1], PCoA2 = pcoa_coords[, 2], row.names = row.names(pcoa_coords))
    pcoa_df
      })
  ...
  pcoa_envfit_react = reactive({
    ...
    meta_data_table <- meta_data_table %>% filter(SampleName %in% pcoa_filt_colnames)
    meta_data_table
    
    # Set any NA values to "NA"
    ...
    pcoa_envfit_df_filt = filter(pcoa_envfit_df, pcoa_envfit_df$pvalue < input$pcoa_env_thresh)
    pcoa_envfit_df_filt
  ...
  pcoa_plot_taxa_fit_react = reactive({
    ...
    taxon_weighted_scores <- wascores(pcoa_srs_plot$vectors[,1:3], pcoa_srs)
    
    taxon_weighted_scores[is.na(taxon_weighted_scores)] <- 0 # remove NA values
    
    # Calculate normalized, total abundance of each taxa
    taxa_total_abundance <- sum(pcoa_srs) #total number of AA asvs in the table 
    taxa_count <- apply(pcoa_srs, 2, sum) #total number of each amoA AA asv across all samples
    normalized_taxa_count <- as.data.frame(taxa_count / taxa_total_abundance)
    colnames(normalized_taxa_count) <- "abundance"
    
    taxon_weighted_scores <- as.data.frame(cbind(taxon_weighted_scores, normalized_taxa_count$abundance)) #Append the abundance information
    colnames(taxon_weighted_scores) <- c("Axis1","Axis2","Axis3","Abundance") # Change column names
    ...
    taxon_weighted_scores <- filter(taxon_weighted_scores, taxon_weighted_scores$Abundance > input$pcoa_taxa_thresh/100) # Filter taxonomy abundance based on a threshold
    taxon_weighted_scores
   
    
  })
  
  pcoa_plot_react = reactive({ ## I need to modify this so if the dataframe is empty it doesn't include the environmental fit data
    ...
    merged_df <- left_join(pcoa_df,meta_data_table,by = "SampleName")
    ...
    pcoa_plot = pcoa_plot + geom_segment(data = pcoa_envfit_df_filt,
                                         aes(x=0,
                                             y=0,
      ...
      geom_label(data = pcoa_envfit_df_filt, aes(label = rownames(pcoa_envfit_df_filt), x = pcoa_envfit_df_filt$axis1/2, y = pcoa_envfit_df_filt$axis2/2),size = 4)
    }
    
      # Do you want to include an ellipsis around your data points?:
      if (input$pcoa_elips == "Yes"){
        pcoa_plot <- pcoa_plot + stat_ellipse(aes(color = get(input$pcoa_fill_col)),
                                                      show.legend = FALSE)
      }
    ...
    pcoa_plot = pcoa_plot + geom_point(data = taxon_weighted_scores,
    ...
    pcoa_plot = pcoa_plot + geom_text(data = taxon_weighted_scores,
    ...
    pcoa_plot <- pcoa_plot +
      theme(
        ...
        axis.line = element_line(colour = "black"),
        axis.text = element_text(colour = "black", size = 12),
        axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5, size = 14, face = "plain"),
  ...
  output$pcoa_download = downloadHandler(
    ...
    content = function(pcoa_file){
      ggsave(pcoa_file, plot = pcoa_plot_re(), device = "pdf", height = as.numeric(input$pcoa_plot_outh), width = as.numeric(input$pcoa_plot_outw), units = "px", scale = 4)
    }
  ) 
  ...
  ranked_abundance_table_re = reactive({
    ...
    find_top_values <- function(column) {
      sorted_column <- sort(column, decreasing = TRUE)  # Sort column in descending order
      threshold <- sum(column) * input$ranked_threshold  # Calculate the threshold representing 90% of all values
      cumsum_values <- cumsum(sorted_column)  # Cumulative sum of sorted values
      top_values <- sorted_column[cumsum_values <= threshold]  # Filter values below the threshold
      return(top_values)
    }
    ...
    for (col_idx in 1:ncol(filtered_table)) {
      ...
      top_values_df[[col_name]] <- ifelse(col_values %in% top_values, col_values, 0)
    }
  ...
  ranked_abundance_bubble_re = reactive({
    ...
    if (is.na(input$ranked_tax_keyword) == TRUE){
      ...
      taxon_hits <- grepl(pattern = paste(input$ranked_tax_keyword), ignore.case = TRUE, x = data_long_bubble$Taxonomy)
      data_long_bubble <- data_long_bubble[taxon_hits,]
      warning("Taxa filtering selected.")
    }
  ...
  ranked_abundance_bubble_plot_re = reactive({
    data_long_bubble <- ranked_abundance_bubble_re()
      ...
      ## Plot the Data:
      bubble_plot <- ggplot(data_long_bubble,
                            aes(x = reorder(SampleName, eval(parse(text = input$ranked_sort_axis))),
                                ...
                                color = as.factor(get(input$ranked_color_param)), size = Percentage),
                            colsep=c(1:100), rowsep=(1:100), sepwidth=c(5,1)) + guides(fill = 'none', colour = 'none')
      ...
      if(input$ranked_second_facet == "No"){
        bubble_plot = bubble_plot + 
          ...
          if(input$ranked_confirm_sort == "Yes"){
            if((input$ranked_facet_side_x == "Top")&(input$ranked_facet_side_y == "Right")){
              facet_nested(eval(parse(text=input$ranked_tax_sort))~eval(parse(text=input$ranked_sort_param)),space = "free",scales="free")
            } else {
              if((input$ranked_facet_side_x == "Bottom")&(input$ranked_facet_side_y == "Left")){
                facet_nested(eval(parse(text=input$ranked_tax_sort))~eval(parse(text=input$ranked_sort_param)),space = "free",scales="free",switch = "both")
              } else {
                if((input$ranked_facet_side_x == "Bottom")&(input$ranked_facet_side_y == "Right")){
                  facet_nested(eval(parse(text=input$ranked_tax_sort))~eval(parse(text=input$ranked_sort_param)),space = "free",scales="free",switch = "x")
                } else {
                  facet_nested(eval(parse(text=input$ranked_tax_sort))~eval(parse(text=input$ranked_sort_param)),space = "free",scales="free",switch = "y")
                }
              }
            }
          } else {
            if(input$ranked_facet_side_x == "Top"){
              facet_nested(~eval(parse(text=input$ranked_sort_param)),space = "free",scales="free")
            } else {
              facet_nested(~eval(parse(text=input$ranked_sort_param)),space = "free",scales="free",switch = "x")
            }}
      }
      ...
      if(input$ranked_second_facet == "Yes"){
        bubble_plot = bubble_plot + 
          ...
          if(input$ranked_confirm_sort == "Yes"){
            if((input$ranked_facet_side_x == "Top")&(input$ranked_facet_side_y == "Right")){
              facet_nested(eval(parse(text=input$ranked_tax_sort)) ~ eval(parse(text=input$ranked_sort_param))+eval(parse(text=input$ranked_second_facet_meta)),space = "free",scales="free")
            } else {
              if((input$ranked_facet_side_x == "Bottom")&(input$ranked_facet_side_y == "Left")){
                facet_nested(eval(parse(text=input$ranked_tax_sort)) ~ eval(parse(text=input$ranked_sort_param))+eval(parse(text=input$ranked_second_facet_meta)),space = "free",scales="free",switch = "both")
              } else {
                if((input$ranked_facet_side_x == "Bottom")&(input$ranked_facet_side_y == "Right")){
                  facet_nested(eval(parse(text=input$ranked_tax_sort)) ~ eval(parse(text=input$ranked_sort_param))+eval(parse(text=input$ranked_second_facet_meta)),space = "free",scales="free",switch = "x")
                } else {
                  facet_nested(eval(parse(text=input$ranked_tax_sort)) ~ eval(parse(text=input$ranked_sort_param))+eval(parse(text=input$ranked_second_facet_meta)),space = "free",scales="free",switch = "y")
                }
              }
            }
          } else {
            if(input$ranked_facet_side_x == "Top"){
              facet_nested(~eval(parse(text=input$ranked_sort_param))+eval(parse(text=input$ranked_second_facet_meta)),space = "free",scales="free")
            } else {
              facet_nested(~eval(parse(text=input$ranked_sort_param))+eval(parse(text=input$ranked_second_facet_meta)),space = "free",scales="free",switch = "x")
            }}
      }
  ...
  output$ranked_bubble_download = downloadHandler(
    ...
    content = function(ranked_file){
      ggsave(ranked_file, plot = ranked_abundance_bubble_plot_re(), device = "pdf", height = as.numeric(input$ranked_plot_out_h), width = as.numeric(input$ranked_plot_out_w), units = "px", scale = 4)
    }
  )   
...
output$ranked_data_table = downloadHandler(
  ...
  ## Populate the drop-down menus with metadata for all figures (need to double check that this isn't resource heavy)
  ## I also need to figure out how to incorporate observeEvent functions that allow me to use "start" buttons and have more control over dynamic interactions
  
  
  
  #### Observe events for populating selectable inputs with metadata
  ...
  # 
  # 
  # observeEvent(input$upload_start,{
  #   ## These lines query the metadata for the column headers and forwards them to the dropdown menus in the ui.

  # 
  # })
  # 
  # observeEvent(input$pcoa_fill_col,{
  #   pcoa_fill_select <- as.character(input$pcoa_fill_col)
  #   print(pcoa_fill_select)
  # })
  # 
